...
def hidratar_sesion_alumno(token, datos_db):
    ...
    # Persistencia en URL: Escribe el token en la barra de direcciones del navegador
...
# ==========================================
# Si el alumno recarga la página pero el token sigue en la URL, se re-autentica automáticamente
...
if st.session_state['auth'] is not None:
    ...
    with col_logout:
        # Botón protegido contra clicks dobles paralelos
        if st.button("Cerrar Sesión 🚪", use_container_width=True, disabled=st.session_state['procesando']):
            ...
            st.session_state['auth'] = None
            st.session_state['token_actual'] = None
            st.query_params.clear()  # Limpia la URL al salir
            st.session_state['procesando'] = False
            st.rerun()


# ==========================================
# VISTA: LOGIN / AUTENTICACIÓN
# ==========================================
...
# ==========================================
# VISTA: CONSOLA DE ADMINISTRACIÓN
# ==========================================
...
# ==========================================
    ...
    # ESTRANGULAMIENTO DE TELEMETRÍA: Solo escribe a la BD si cambió el valor del minuto actual
    if minutos_esta_sesion != st.session_state['ultimo_minuto_sincronizado']:
        try:
            ...
            pass  # Resiliencia: si falla la red, el flujo continúa sin interrumpir al alumno
    
    # Renderizado estático estable de métricas superiores
    ...
    # Bloque de seguridad clínica por pérdida de vidas
