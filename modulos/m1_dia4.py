...
def mostrar_dia4():
    ...
    if st.button("Inyectar 10 mL de HCl", use_container_width=True):
        ...
        if "Agua" in solucion:
            ...
            st.markdown("<div class='card-success'>🛡️ <b>TAMPÓN EXITOSO:</b> El bicarbonato absorbió los protones ($H^+$) sin alterar el equilibrio celular.</div>", unsafe_allow_html=True)
            st.session_state["advertencia_ph"] = False
    st.markdown("</div>", unsafe_allow_html=True)
    
    st.markdown("<div class='lab-panel'>", unsafe_allow_html=True)
    st.markdown("### 🚑 Cuestionario de Certificación Fisiológica")
    Q1 = st.radio("1. ¿Por qué la evolución orgánica seleccionó la D-Glucosa sobre la L-Glucosa?", ["A) Desvía la luz a la derecha.", "B) Modelo estereoquímico llave-cerradura en los sitios activos de las enzimas."], index=None)
    Q2 = st.radio("2. Glucosa y Galactosa difieren únicamente en la orientación espacial del carbono asimétrico 4 (C-4), por lo tanto son:", ["A) Isótopos", "B) Epímeros"], index=None)
