...
def mostrar_dia4():
    ...
    if "Carbonilo" in group: st.warning("**Carbonilo:** Centro de glúcidos.")
    ...
    if st.button("Inyectar 10 mL de HCl", use_container_width=True):
        ...
        if "Agua" in solucion:
            ...
            st.markdown("<div class='card-success'>🛡️ <b>TAMPÓN EXITOSO:</b> El bicarbonato absorbió los protones.</div>", unsafe_allow_html=True)
            st.session_state["advertencia_ph"] = False
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    if st.button("Firmar y Evaluar Módulo", use_container_width=True):
        ...
        if not Q1 or not Q2: 
            ...
            if st.session_state["errores_quiz"] == 1:
                ...
                st.markdown(f"<div class='card-hint'>💡 Tienes {errores} error(es). Intenta corregir.</div>", unsafe_allow_html=True)
            else:
                st.session_state["vidas"] = max(0, st.session_state["vidas"] - 1)
                db.registrar_intento_quiz(token, nuevos_errores=errores, nuevas_vidas=st.session_state["vidas"], tiempo_seg=st.session_state['tiempo_estudio_seg'])
